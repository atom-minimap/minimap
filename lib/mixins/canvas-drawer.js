...
export default class CanvasDrawer extends Mixin {
   ...
   */
  getDefaultColor () {
    const color = this.DOMStylesReader.retrieveStyleFromDom(['.editor'], 'color', true)
  ...
  getTokenColor (token) {
    ...
    const color = this.DOMStylesReader.retrieveStyleFromDom(scopes, 'color')
  ...
  getDecorationColor (decoration) {
    ...
    if (properties.scope) {
      ...
      return this.DOMStylesReader.retrieveStyleFromDom(scopeString, 'background-color')
    } else {
