...
export default class CanvasDrawer extends Mixin {
   ...
   */
  getFrontCanvas () { return this.canvasLayer.canvas }
  ...
  attachCanvases (parent) {
    this.canvasLayer.attach(parent)
  }

  setCanvasesSize (width, height) {
    this.canvasLayer.setSize(width, height)
  }
  ...
  updateCanvas () {
    ...
    let context = this.canvasLayer.context
    ...
    if (intactRanges.length === 0) {
      ...
      for (let j = 0, len = intactRanges.length; j < len; j++) {
        ...
        this.canvasLayer.copyPartFromOffscreen(
    ...
    this.canvasLayer.resetOffscreenSize()
    this.canvasLayer.copyToOffscreen()
  ...
  drawLines (context, firstRow, lastRow, offsetRow) {
    ...
    let {width: canvasWidth, height: canvasHeight} = this.canvasLayer.getSize()
