...
module.exports = class CanvasDrawer extends Mixin {
  ...
  initializeCanvas () {
    ...
    */
    ...
    this.maxTokensInOneLine = atom.config.get("minimap-plus.maxTokensInOneLine")
  }
  ...
  redrawRangesOnLayer (layer, intactRanges, firstRow, lastRow, method) {
    ...
    if (intactRanges.length === 0) {
      // 10-100 called when editor changes (not in scroll) or initially
  ...
  drawLines (firstRow, lastRow, offsetRow) {
    ...
    // 10-100ms (higher initially. triggered with editor edits)
    let lastLine, x
