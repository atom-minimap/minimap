...
export default class CanvasDrawer extends Mixin {
  ...
  updateCanvas () {
    ...
    const pluginsOrder = Main.getPluginsOrder()

    updateBackDecorationsLayer(this.backLayer, firstRow, lastRow, this.offscreenFirstRow, this.offscreenLastRow, this.pendingBackDecorationChanges, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)

    updateFrontDecorationsLayer(this.frontLayer, firstRow, lastRow, this.offscreenFirstRow, this.offscreenLastRow, this.pendingFrontDecorationChanges, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)
 ...
 * @param {object} pluginsOrder
 * @access private
 */
function updateBackDecorationsLayer (backLayer, firstRow, lastRow, offscreenFirstRow, offscreenLastRow, pendingBackDecorationChanges, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder) {
  ...
  if (intactRanges.length === 0) {
    drawBackDecorationsForLines(context, firstRow, lastRow, 0, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)
  } else {
    for (let j = 0, len = intactRanges.length; j < len; j++) {
      const intact = intactRanges[j]
    ...
    for (let i = 0, len = intactRanges.length; i < len; i++) {
      ...
      drawBackDecorationsForLines(context, currentRow, range.start, currentRow - firstRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)

      currentRow = range.end
    }
    if (currentRow <= lastRow) {
      drawBackDecorationsForLines(context, currentRow, lastRow, currentRow - firstRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)
    }
  }
 ...
 * @access private
 */
function updateFrontDecorationsLayer (frontLayer, firstRow, lastRow, offscreenFirstRow, offscreenLastRow, pendingFrontDecorationChanges, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder) {
  ...
  if (intactRanges.length === 0) {
    drawFrontDecorationsForLines(context, firstRow, lastRow, 0, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)
  } else {
    for (let j = 0, len = intactRanges.length; j < len; j++) {
      const intact = intactRanges[j]
    ...
    for (let i = 0, len = intactRanges.length; i < len; i++) {
      ...
      drawFrontDecorationsForLines(context, currentRow, range.start, currentRow - firstRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)

      currentRow = range.end
    }
    if (currentRow <= lastRow) {
      drawFrontDecorationsForLines(context, currentRow, lastRow, currentRow - firstRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder)
    }
  }
 ...
 * @access private
 */
function drawDecorations (context, screenRow, yRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, decorations, editorElement, pluginsOrder) {
 ...
 * @access private
 */
function drawFrontDecorationsForLines (context, firstRow, lastRow, offsetRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder) {
  if (firstRow > lastRow) { return }

  for (let screenRow = firstRow; screenRow <= lastRow; screenRow++) {
    const yRow = (offsetRow + (screenRow - firstRow)) * lineHeight // row * lineHeight

    drawDecorations(context, screenRow, yRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, decorations, editorElement, pluginsOrder)
  }
 ...
 * @access private
 */
function drawBackDecorationsForLines (context, firstRow, lastRow, offsetRow, lineHeight, charHeight, charWidth, canvasHeight, canvasWidth, editorElement, decorations, pluginsOrder) {
  if (firstRow > lastRow) { return }
