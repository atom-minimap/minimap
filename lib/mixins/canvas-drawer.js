...
export default class CanvasDrawer extends Mixin {
  ...
  updateCanvas () {
    ...
    updateTokensLayer(this.tokensLayer, firstRow, lastRow, this.offscreenFirstRow, this.offscreenLastRow, this.pendingChanges, lineHeight, charHeight, charWidth, canvasWidth, canvasHeight, editor, editor.getScreenLineCount(), getInvisibleRegExp(editor), getTokenColorClosure, this.ignoreWhitespacesInTokens, this.maxTokensInOneLine)
 ...
 * @access private
 */
function updateTokensLayer (tokensLayer, firstRow, lastRow, offscreenFirstRow, offscreenLastRow, pendingChanges, lineHeight, charHeight, charWidth, canvasWidth, canvasHeight, editor, editorScreenLineCount, invisibleRegExp, getTokenColorClosure, ignoreWhitespacesInTokens, maxTokensInOneLine) {
...
function drawLines (firstRow, lastRow, offsetRow, lineHeight, charHeight, charWidth, canvasWidth, context, editor, editorScreenLineCount, invisibleRegExp, getTokenColorClosure, ignoreWhitespacesInTokens, maxTokensInOneLine) {
  ...
  for (let line = firstRow; line < lastRow; line++) {
    ...
    for (let iToken = 0; iToken < numTokenToRender; iToken++) {
      ...
      const width = tokenText.length * charWidth

      if (whitespaceTokenRegex.test(tokenText)) {
        x += width
      } else {
        buffer = new Uint8ClampedArray(4 * width * charHeight)
        const [r, g, b] = getTokenColorClosure(tokenScopes)
        let xi = 0
        while (xi < width) {
            let yi = 0
            while (yi < charHeight) {
                // RHS: RGBA little endian color
                buffer[width * xi + yi] = r
                buffer[width * xi + yi + 1] = g
                buffer[width * xi + yi + 2] = b
                buffer[width * xi + yi + 3] = 0
                yi++
            }
            xi++
        }
        context.putImageData(new ImageData(buffer, width), x, y);
        x = x + width
      }
    }
  }
 ...
 * @param {TextEditorElement} editorElement
 * @param {number} textOpacity
 * @return {number} the CSS color for the provided token
 */
function getTokenColor (scopes, editorElement, textOpacity) {
  const colorDecimal = domStylesReader.retrieveStyleFromDom(scopes, 'color', editorElement, true)
  // return color
  return colorDecimal
  // return transparentize(color, textOpacity)
}
