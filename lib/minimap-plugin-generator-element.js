...
class MinimapPluginGeneratorElement {
  ...
  confirm () {
    if (this.validPackagePath()) {
      ...
      this.createPackageFiles(() => {
        ...
        atom.open({pathsToOpen: [packagePath], devMode: atom.config.get('minimap-plus.createPluginInDevMode')})
  ...
  linkPackage (packagePath, callback) {
    let args = ['link']
    if (atom.config.get('minimap-plus.createPluginInDevMode')) { args.push('--dev') }
