...
export default class MinimapElement {
      ...
      'minimap.smoothScrolling': (smoothScrolling) => {
        ...
        if (this.attached) {
          if (!this.smoothScrolling) {
            ...
            this.overlayLayer.canvas.style.cssText = ''
          } else {
            this.requestUpdate()
          }
        }
  ...
  setModel (minimap) {
    ...
    this.subscriptions.add(this.minimap.onDidChangeDecorationRange((change) => {
      const {type} = change
      if (type === 'overlay') {
        this.pendingOverlayDecorationChanges.push(change)
      } else if (type === 'line' || type === 'highlight-under') {
  ...
  update () {
    ...
    if (this.smoothScrolling) {
      if (SPEC_MODE) {
        ...
        this.applyStyles(this.overlayLayer.canvas, {top: canvasTop + 'px'})
      } else {
        ...
        this.applyStyles(this.overlayLayer.canvas, {transform: canvasTransform})
      }
    }
