...
class MinimapElement {
  ...
  createdCallback () {
    ...
    */
    ...
    return this.observeConfig({
      'minimap-plus.displayMinimapOnLeft': (displayMinimapOnLeft) => {
      ...
      'minimap-plus.minimapScrollIndicator': (minimapScrollIndicator) => {
      ...
      'minimap-plus.displayPluginsControls': (displayPluginsControls) => {
      ...
      'minimap-plus.textOpacity': (textOpacity) => {
        this.textOpacity = textOpacity

        if (this.attached) { this.requestForcedUpdate() }
      },

      'minimap-plus.displayCodeHighlights': (displayCodeHighlights) => {
        this.displayCodeHighlights = displayCodeHighlights

        if (this.attached) { this.requestForcedUpdate() }
      },

      'minimap-plus.smoothScrolling': (smoothScrolling) => {
      ...
      'minimap-plus.adjustMinimapWidthToSoftWrap': (adjustToSoftWrap) => {
        this.adjustToSoftWrap = adjustToSoftWrap

        if (this.attached) { this.measureHeightAndWidth() }
      },

      'minimap-plus.adjustMinimapWidthOnlyIfSmaller': (adjustOnlyIfSmaller) => {
        this.adjustOnlyIfSmaller = adjustOnlyIfSmaller

        if (this.attached) { this.measureHeightAndWidth() }
      },

      'minimap-plus.useHardwareAcceleration': (useHardwareAcceleration) => {
        this.useHardwareAcceleration = useHardwareAcceleration

        if (this.attached) { this.requestUpdate() }
      },

      'minimap-plus.absoluteMode': (absoluteMode) => {
        this.absoluteMode = absoluteMode

        this.classList.toggle('absolute', this.absoluteMode)
      },

      'minimap-plus.adjustAbsoluteModeHeight': (adjustAbsoluteModeHeight) => {
      ...
      'minimap-plus.ignoreWhitespacesInTokens': (ignoreWhitespacesInTokens) => {
  ...
  canvasLeftMousePressed (y) {
    ...
    if (atom.config.get('minimap-plus.moveCursorOnMinimapClick')) {
      textEditor.setCursorScreenPosition([row, 0])
    }

    if (atom.config.get('minimap-plus.scrollAnimation')) {
      const duration = atom.config.get('minimap-plus.scrollAnimationDuration')
