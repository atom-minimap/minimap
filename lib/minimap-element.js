...
export default class MinimapElement {
  ...
  update () {
    ...
    if (SPEC_MODE) {
      this.applyStyles(canvas, {top: canvasTop + 'px'})
    } else {
      this.applyStyles(canvas, {transform: canvasTransform})
    }
  ...
  measureHeightAndWidth (visibilityChanged, forceUpdate = true) {
    ...
    if (wasResized || forceUpdate) {
      ...
      if (canvasWidth !== canvas.width || this.height !== canvas.height) {
        ...
        canvas.height = (this.height + this.minimap.getLineHeight()) * devicePixelRatio
      }
    }
  }
