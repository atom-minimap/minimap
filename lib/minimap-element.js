...
export default class MinimapElement {
  ...
  getTextEditorElementRoot () {
    const editorElement = this.getTextEditorElement()
  ...
  update () {
    ...
    if (this.scrollIndicator != null) {
      ...
      const indicatorScroll = (minimapScreenHeight - indicatorHeight) * minimap.getCapedTextEditorScrollRatio()
  ...
  leftMousePressedOverCanvas ({pageY, target}) {
    const y = pageY - target.getBoundingClientRect().top
    const row = Math.floor(y / this.minimap.getLineHeight()) + this.minimap.getFirstVisibleScreenRow()
    ...
    if (atom.config.get('minimap.scrollAnimation')) {
      ...
      const to = scrollTop
      const step = (now) => this.minimap.setTextEditorScrollTop(now)
      const duration = atom.config.get('minimap.scrollAnimationDuration')
      this.animate({ from, to, duration, step })
    } else {
  ...
  middleMousePressedOverCanvas ({pageY}) {
    ...
    const y = pageY - offsetTop - this.minimap.getTextEditorScaledHeight() / 2

    const ratio = y / (this.minimap.getVisibleHeight() - this.minimap.getTextEditorScaledHeight())
  ...
  startDrag (e) {
    ...
    const dragOffset = pageY - top

    const initial = { dragOffset, offsetTop }

    const mousemoveHandler = (e) => this.drag(e, initial)
    const mouseupHandler = (e) => this.endDrag(e, initial)
  ...
  drag (e, initial) {
    ...
    const y = e.pageY - initial.offsetTop - initial.dragOffset
   ...
   *
   * http://jsperf.com/new-date-vs-date-now/5
   *
