...
class Minimap {
  ...
  subscribeToConfig () {
    ...
    subs.add(atom.config.observe('minimap-plus.charHeight', opts, (configCharHeight) => {
      this.configCharHeight = configCharHeight
      this.updateScrollTop()
      this.emitter.emit('did-change-config')
    }))
    subs.add(atom.config.observe('minimap-plus.charWidth', opts, (configCharWidth) => {
      this.configCharWidth = configCharWidth
      this.updateScrollTop()
      this.emitter.emit('did-change-config')
    }))
    subs.add(atom.config.observe('minimap-plus.interline', opts, (configInterline) => {
      this.configInterline = configInterline
      this.updateScrollTop()
      this.emitter.emit('did-change-config')
    }))
    subs.add(atom.config.observe('minimap-plus.independentMinimapScroll', opts, (independentMinimapScroll) => {
      this.independentMinimapScroll = independentMinimapScroll
      this.updateScrollTop()
    }))
    subs.add(atom.config.observe('minimap-plus.scrollSensitivity', opts, (scrollSensitivity) => {
      this.scrollSensitivity = scrollSensitivity
    }))
    subs.add(atom.config.observe('minimap-plus.redrawDelay', opts, (redrawDelay) => {
      this.redrawDelay = redrawDelay
    }))
    // cdprr is shorthand for configDevicePixelRatioRounding
    subs.add(atom.config.observe(
      'minimap-plus.devicePixelRatioRounding',
      opts,
      (cdprr) => {
