...
/**
 * This class is used by the `CanvasDrawer` in `MinimapElement` to read the styles informations (color and
 * background-color) from the DOM to use when rendering the `Minimap`.
 *
 * It attaches a dummyNode to the targetNode, renders them, and finds the computed style back. TODO: find a better way
 * to get the token colors
 */
export default class DOMStylesReader {
   ...
   *
   * @param {string[]} scopes A list of classes reprensenting the scope to build
   ...
   * @returns {string} The computed property's value used in CanvasDrawer
   */
 ...
 * @returns {string} The rotated CSS color
 * @access private
 */
 ...
 * @returns {number[]} The rotated color channels
 * @access private
 */
