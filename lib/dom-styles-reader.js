...
export default class DOMStylesReader {
  ...
  retrieveStyleFromDom (scopes, property, targetNode, getFromCache) {
    if (!scopes.length) { return 0 } // no scopes
    ...
    const value_str = style.getPropertyValue(property)

    const [, , r_str, g_str, b_str, , a_str] = value_str.match(rgbExtractRegexp)
    const value = [r_str, g_str, b_str, a_str].map((n) => parseInt(n, 10) | 0)
