...
class MinimapQuickSettingsElement {
  ...
  setModel (minimap) {
    ...
    subs.add(this.subscribeTo(this.codeHighlights, {
      'mousedown': (e) => {
        e.preventDefault()
        atom.config.set('minimap-plus.displayCodeHighlights', !this.minimap.displayCodeHighlights)
      }
    }))
    ...
    subs.add(this.subscribeTo(this.absoluteMode, {
      'mousedown': (e) => {
        e.preventDefault()
        atom.config.set('minimap-plus.absoluteMode', !atom.config.get('minimap-plus.absoluteMode'))
      }
    }))
    ...
    subs.add(this.subscribeTo(this.adjustAbsoluteModeHeight, {
      'mousedown': (e) => {
        e.preventDefault()
        atom.config.set('minimap-plus.adjustAbsoluteModeHeight', !atom.config.get('minimap-plus.adjustAbsoluteModeHeight'))
      }
    }))
    ...
    subs.add(atom.config.observe('minimap-plus.displayCodeHighlights', (bool) => {
      this.codeHighlights.classList.toggle('active', bool)
    }))

    subs.add(atom.config.observe('minimap-plus.absoluteMode', (bool) => {
      this.absoluteMode.classList.toggle('active', bool)
    }))

    subs.add(atom.config.observe('minimap-plus.adjustAbsoluteModeHeight', (bool) => {
      this.adjustAbsoluteModeHeight.classList.toggle('active', bool)
    }))

    subs.add(atom.config.observe('minimap-plus.displayMinimapOnLeft', (bool) => {
