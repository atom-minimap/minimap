...
class Main {
  ...
  activate () {
    ...
    this.subscriptionsOfCommands = atom.commands.add('atom-workspace', {
      ...
      'minimap-plus:generate-coffee-plugin': () => {
        this.generatePlugin('coffee')
      },
      'minimap-plus:generate-javascript-plugin': () => {
        this.generatePlugin('javascript')
      },
      'minimap-plus:generate-babel-plugin': () => {
    ...
    if (atom.config.get('minimap-plus.autoToggle')) { this.toggle() }
  }
  ...
  getConfigSchema () {
    return this.config
      ? this.config
      : atom.packages.getLoadedPackage('minimap-plus').metadata.configSchema
  }
